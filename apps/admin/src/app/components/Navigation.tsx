        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              ...
              >
                <motion.span
                  className={`text-base uppercase tracking-wide transition-colors ${location.pathname === link.path
                      ? 'text-[#FFBD01]'
                ...
                <div className="flex-1 py-8 px-6 space-y-2 overflow-y-auto">
                  {navLinks.map((link) => (
                    <Link
                      ...
                      to={link.path}
                      onClick={() => setMobileMenuOpen(false)}
                      className={`block py-4 px-5 text-lg font-medium uppercase tracking-wide transition-all rounded-2xl ${location.pathname === link.path
