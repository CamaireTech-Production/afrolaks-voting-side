...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        ...
        voteData: { nomineeId, categoryId, voteCount, amountXAF: finalAmountXAF, transactionId, paymentStatus },
      },
      { status: 201 }
    );
  } catch (error) {
    ...
    return NextResponse.json(
      { error: 'Failed to record vote', details: (error as Error).message },
      { status: 500 }
    );
  }
}
